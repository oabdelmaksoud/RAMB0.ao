...
export default function WorkflowCanvas() {
  ...
  const handleDrop = (event: DragEvent<HTMLDivElement>) => {
    ...
    const agentCardHeight = 100; // Approximate height of the card
    
    const adjustedX = Math.min(Math.max(0, x - agentCardWidth / 2), canvasRect.width - agentCardWidth);
    const adjustedY = Math.min(Math.max(0, y - agentCardHeight / 2), canvasRect.height - agentCardHeight);
  ...
  return (
    ...
    >
      ...
      {droppedAgents.map((agent) => (
        <Card 
          key={agent.id} 
          className="absolute w-[180px] shadow-lg cursor-grab bg-card"
