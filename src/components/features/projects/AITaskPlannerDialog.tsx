...
import type { ProjectWorkflow } from '@/types'; // Ensure this is the correct type
...
const plannerSchema = z.object({
  ...
  selectedWorkflowId: z.string().optional(), // ID of the workflow selected by user to guide AI
});
...
export default function AITaskPlannerDialog({
  ...
  const [feedbackText, setFeedbackText] = React.useState(""); // For modification feedback
  ...
  const prepareAndRunPlanner = async (currentGoal: string, modificationDetails?: { request: string; previousPlan: PlanProjectTaskOutput }) => {
    setIsLoading(true);
    if (!modificationDetails) { // Only clear suggestion if it's a new plan, not a modification
      setAiSuggestion(null);
    }
    setSelectedWorkflowOverrideId(undefined); // Reset override for new plans or modifications
    ...
    try {
      ...
      if (!modificationDetails) setAiSuggestion(null); // Only clear if it wasn't a modification attempt
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAcceptAndAddTask = () => {
    if (aiSuggestion?.plannedTask) {
      ...
      const userSelectedWorkflowForOverride = form.getValues("selectedWorkflowId");
      ...
      <DialogContent className="sm:max-w-[600px] md:max-w-[750px] lg:max-w-[800px] flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-4">
          ...
          {aiSuggestion && aiSuggestion.plannedTask && !isLoading && (
             <ScrollArea className="max-h-[calc(90vh-250px)]"> {/* Adjust max-h as needed */}
          ...
          {aiSuggestion && (
            <>
              <Button variant="outline" onClick={() => {
                form.reset({ userGoal: form.getValues("userGoal"), selectedWorkflowId: form.getValues("selectedWorkflowId") }); // Keep goal for modification
                setAiSuggestion(null);
                setSelectedWorkflowOverrideId(undefined);
                setFeedbackText("");
                setIsLoading(false);
